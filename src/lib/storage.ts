import type { AppState, BudgetMin, LessonRecord, LessonStatus } from './types';
...
// Best-effort migration map from v1 day numbers → v2 lesson ids.
// Mirrors the final v1 77-day schedule. Rest days are dropped (no equivalent lesson).
...
function migrateFromV1(): AppState {
  ...
  for (const [dayStr, day] of Object.entries(v1.days)) {
    ...
    for (const id of ids) {
      ...
      // Don't downgrade 'done' to 'skipped' if already set
  ...
  return { version: 2, records, budgetMin: DEFAULT_BUDGET };
}
